...
//           log.info("]" + p.getBody() + "[");
            ...
            /*
             * InputStream is = FileUtils.readResource(GitHubGantt.class,
             * "githubgantt/gantt-dhtmlx.html");
             * 
             * byte[] html = FileUtils.toByteArray(is);
             * 
             * String htmlOut = TemplateEngine.processExpressionLanguage(new String(html,
             * "utf8"), parameters);
             * 
             * FileWriter fos = new FileWriter("index.html");
             * 
             * fos.write(htmlOut);
             * 
             * fos.close();
            ...
            log.info("FINAL " + (System.currentTimeMillis() - start) + " ms");
        } catch (Exception e) {
    ...
    public Map<String, Object> generateGanttData(String organization, String repository, long projectID,
        ...
        for (GHIssue i : issues) {
            ...
            if (isCAIB && isEPIC) {
                errors.add(encodeHtml("La tasca " + i.getNumber() + " te marcat CAIB i EPIC alhora !!!")
                        + "<a target=\"_blank\" href=\"https://github.com/" + organization + "/" + repository + "/issues/" + i.getNumber()
                        + "\"> #" + i.getNumber() + "</a> - " + (i.getAssignees().isEmpty()?"":i.getAssignees().get(0).getLogin()));
                continue;
            }
            ...
            if (!isCAIB && dependency == null) {
                errors.add(encodeHtml("La tasca " + i.getNumber()
                        + " no depen de cap Issue. "
                        + "Ha de dependre d'un issue EPIC o CAIB i ha d´incloure el text 'Depèn de [text_issue] #[num issue]'")
                        + "<a target=\"_blank\" href=\"https://github.com/" + organization + "/" + repository + "/issues/" + i.getNumber()
                        + "\"> #" + i.getNumber() + "</a> - " + (i.getAssignees().isEmpty()?"":i.getAssignees().get(0).getLogin()));
                continue;
            }
        ...
        if (errors.size() == 0) {
            ...
            gantts.add(new GanttItem(1, " P R O J E C T   " + projectName, null,
                    new Date(startDate.getTime() - 2 * 60 * 60 * 1000), null, null, 0, 0, true, true));

            generateDhtmlxGanttData(parameters, startDate, gantts, durationsEpicCAIB, programadors, errors, warnings, repository, organization);
        }
    ...
    protected void generateDhtmlxGanttData(Map<String, Object> parameters, Date startDate, List<GanttItem> gantts,
            Map<Integer, Long> durationsEpicCAIB, int programadors, List<String> errors, List<String> warnings, String repository,String organization)
            throws IOException {
        ...
        for (GanttItem gi : gantts) {
            ...
            if (pare == null || pare == 1) {
                ...
                if (map.get(gi.getDependency()) == null) {
                    errors.add(encodeHtml("La tasca " + gi.getTaskID() + "  depen de una tasca (" + gi.getDependency()
                            + ") que no existeix dins del projecte'") + "<a target=\"_blank\" href=\"https://github.com/" + organization + "/" + repository + "/issues/" + gi.getTaskID()
                            + "\"> #" + gi.getTaskID() + "</a> - <a target=\"_blank\" href=\"https://github.com/" + organization + "/" + repository + "/issues/" +  gi.getDependency()
                            + "\"> #" +  gi.getDependency() + "</a> " );
                    continue;
                } else {
            ...
            if (gi.isCaib() || (gi.getDependency() != null && gi.getDependency() == 1)) {
        ...
        if (errors.size() == 0) {
            ...
            for (Integer caibID : tasquesCAIB) {
                ...
                links.add(new GanttLink(1, parent.getTaskID(), GanttLink.START_TO_START));
        ...
        map.get(1).setPercentCompleted(percent / tasquesCAIB.size());
        ...
        for (GanttItem gi : gantts) {
            ...
            str.append("{id:" + gi.getTaskID() + ", text:\""
                    + encodeHtml(gi.getTaskName().replace('"', '\'')
                            + ((gi.getTaskID() == 1) ? "" : (" #" + gi.getTaskID())))
                    + "\"" + ", start_date:" + dateToDhtmlx(gi.getStart()) + ", duration:"
                    + (gi.getDuration() == null ? "null" : (gi.getDuration())) + ", parent:"
                    + (gi.getDependency() == null ? 1 : gi.getDependency()) + ", progress:"
                    + (gi.getPercentCompleted() / 100f) + ", color: \"#" + gi.getColor() + "\"}\n");
