...
<un:useConstants var="FitxerFields" className="org.fundaciobit.githubgantt.model.fields.FitxerFields"/>
        ...
        <th>${ghg:getSortIcons(__theFilterForm,FitxerFields.FITXERID)}</th>
        </c:if>
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,FitxerFields.NOM)}">
        <th>${ghg:getSortIcons(__theFilterForm,FitxerFields.NOM)}</th>
        </c:if>
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,FitxerFields.MIME)}">
        <th>${ghg:getSortIcons(__theFilterForm,FitxerFields.MIME)}</th>
        </c:if>
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,FitxerFields.TAMANY)}">
        <th>${ghg:getSortIcons(__theFilterForm,FitxerFields.TAMANY)}</th>
        </c:if>
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,FitxerFields.DESCRIPCIO)}">
        <th>${ghg:getSortIcons(__theFilterForm,FitxerFields.DESCRIPCIO)}</th>
        </c:if>
