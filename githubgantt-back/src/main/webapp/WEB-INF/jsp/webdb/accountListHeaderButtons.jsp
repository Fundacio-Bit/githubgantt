...
<%@ include file="/WEB-INF/jsp/moduls/includes.jsp"%>
  ...
  <c:if test="${fn:length(groupby_items) > 0}">
    ...
    <c:if test="${__theFilterForm.deleteSelectedButtonVisible && __theFilterForm.visibleMultipleSelection && not empty accountItems}">
      <a class="btn btn-danger btn-sm float-right" style="color: white;" href="#myModal"
        onclick="openModalSubmit('<c:url value="${contexte}/deleteSelected"/>','show', 'account');"
