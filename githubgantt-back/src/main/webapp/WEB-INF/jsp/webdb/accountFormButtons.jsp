...
<%@ include file="/WEB-INF/jsp/moduls/includes.jsp"%>
  ...
  <div class="navbar-form float-right">
    ...
    <input type="button" class="btn" onclick="goTo('<c:url value="${contexte}/${__theForm.account.accountID}/cancel"/>')" value="<fmt:message key="genapp.cancel"/>">
    </c:if>
    <c:if test="${!__theForm.nou && __theForm.deleteButtonVisible}">
    <button type="button" class="btn btn-danger" onclick="openModal('<c:url value="${contexte}/${__theForm.account.accountID}/delete"/>','show');"><fmt:message key="genapp.delete"/></button>
    </c:if>
    ...
    <c:set var="pk" value="${__theForm.account.accountID}"/>
