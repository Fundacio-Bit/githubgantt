...
<%@ include file="/WEB-INF/jsp/moduls/includes.jsp"%>
          ...
          <td>
          <c:set var="pk" value="${gantt.ganttID}"/>
          <c:choose>
           <c:when test="${__theFilterForm.actionsRenderer == 1}">
            <div class="btn-group" role="group" >
            <c:if test="${__theFilterForm.editButtonVisible}">
            <a class="btn btn-warning" href="<c:url value="${contexte}/${gantt.ganttID}/edit"/>" role="button"  title="<fmt:message key="genapp.edit"/>">
               <i class="fas fa-edit"></i>
            </a>
            </c:if>
            <c:if test="${__theFilterForm.deleteButtonVisible}">
            <a class="btn btn-danger" href="#myModal" role="button"  onclick="openModal('<c:url value="${contexte}/${gantt.ganttID}/delete"/>','show');" title="<fmt:message key="genapp.delete"/>">
      ...
      <ul class="dropdown-menu float-right" style="min-width:35px;padding:5px 5px 0px 5px;margin:0px;font-size: 12px" >
            <c:if test="${__theFilterForm.editButtonVisible}">
            <li>
            <a class="btn btn-warning btn-sm a_item" style="margin-bottom:5px;color: white;" href="<c:url value="${contexte}/${gantt.ganttID}/edit"/>" onclick="null">
            <i class="fas fa-edit"></i>
            ...
            <c:if test="${__theFilterForm.deleteButtonVisible}">
            <li>
            <a class="btn btn-danger btn-sm a_item" style="margin-bottom:5px;color: white;" href="#myModal" onclick="openModal('<c:url value="${contexte}/${gantt.ganttID}/delete"/>','show');">
            <i class="fas fa-trash icon-white"></i>
